    ...
    def get(self, request):
        if request.user.is_superuser:
            projects = Project.objects.annotate(
                doc_count=Count("documents"), member_count=Count("memberships")
            )
        else:
...
class ProjectDetailView(LoginRequiredMixin, View):
    def get(self, request, pk):
        ...
        my_assignments = (
            Assignment.objects.filter(project=project, annotator=request.user).select_related(
                "document"
            )
            if not admin
            else None
...
class RISImportView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):
        project = self._setup(request, pk)
        return render(
            request, "projects/ris_import.html", {"project": project, "form": RISImportForm()}
        )
...
class PDFUploadView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):
        project = self._setup(request, pk)
        return render(
            request, "projects/pdf_upload.html", {"project": project, "form": PDFUploadForm()}
        )
...
class DocumentDetailView(LoginRequiredMixin, View):
    def get(self, request, pk, doc_pk):
        ...
        assignments = Assignment.objects.filter(document=doc).select_related("annotator")
        return render(
            request,
...
class TimeReportView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        resp["Content-Disposition"] = f'attachment; filename="time-report-project-{pk}.csv"'
        w = csv.writer(resp)
        w.writerow(
            ["document_id", "document_title", "annotator",
             "sessions", "active_seconds", "active_minutes", "open_seconds"]
        )
        for r in rows:
...
class IRRExportView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        edges = (
            ...
            .order_by("graph__document__title", "graph__annotator__username", "-created_at")
        )

        resp = HttpResponse(content_type="text/csv; charset=utf-8")
        resp["Content-Disposition"] = f'attachment; filename="irr-export-project-{pk}.csv"'
        w = csv.writer(resp)
        w.writerow([
            "document_id", "document_title", "graph_id", "annotator",
            "edge_id", "edge_status", "subject_name", "object_name",
            "predicate", "claim_strength", "philosophical_account", "certainty_grade",
        ])
        for edge in edges:
            data = edge.data
            w.writerow([
